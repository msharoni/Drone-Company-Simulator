...
namespace DalObject
{
    class DataSource
    {
        ...
        public static void Intalize()
        {
            ...
            for (int i = 0; i < 5; ++i)
                Drones.Add(new Drone { Id = r.Next(100), Model = "Model" + r.Next(100).ToString(), MaxWeight = (WeightCategories)r.Next(1,3) });
            for (int i = 0; i < 2; ++i)
            ...
            for (int i = 0; i < 10; ++i)
                Parcels.Add(new Parcel { Id = r.Next(99999999, 1000000000),SenderId = Customers[r.Next(0, Customers.Count)].Id, TargetId = Customers[r.Next(0, Customers.Count)].Id, Weight = (WeightCategories)r.Next(3), Priority = (Priorities)r.Next(3), Requested = DateTime.Now, DroneId = r.Next(100), Scheduled = DateTime.Now, PickedUp = new DateTime(), Delivered = new DateTime() });
        }
        internal class Config
        {
            //the first place is charge speed second is no weight flight third is light weight...
            internal static double[] batteryPerKm = { 0.1, 0.002, 0.003, 0.004, 0.005 };
